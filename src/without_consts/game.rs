...
#[derive(Clone)]
pub struct Game<'a> {
    game_board: &'a [&'a [&'a Player]],
    win_conditions: &'a Vec<Box<dyn WinCondition<'a>>>,
...
impl<'a> Game<'a> {
    ...
    fn place_piece(&mut self, column: usize, current: &'a Player) -> Result<(), &str> {
        for row in 0..self.game_board[column].len() {
            if self.game_board[column][row] == &Player::None {
                self.game_board[column] = current;
                return Ok(());
            }
        }
        Err("Column is full")
